...
@pytest.mark.asyncio
async def test_create_calendar_event_default_calendar(temp_calendar_path):
...
@pytest.mark.asyncio
async def test_create_calendar_event_custom_calendar(temp_calendar_path):
...
@pytest.mark.asyncio
async def test_create_calendar_event_invalid_date(temp_calendar_path):
...
@pytest.mark.asyncio
async def test_list_calendar_events_empty(temp_calendar_path):
    """Test listing events from non-existent calendar."""
    result = await list_calendar_events(calendar_name="nonexistent")
    assert "Calendar 'nonexistent' does not exist" in result


@pytest.mark.asyncio
async def test_list_calendar_events_with_date_filter(temp_calendar_path):
    ...
    result = await list_calendar_events(start_date="2025-12-01", end_date="2025-12-20")
...
async def test_delete_calendar_event(temp_calendar_path):
    ...
    # Delete event
    result = await delete_calendar_event(event_title="Test Event")
...
@pytest.mark.asyncio
async def test_delete_calendar_event_not_found(temp_calendar_path):
    """Test deleting a non-existent event."""
    # Create a calendar first
    await create_calendar_event(event_title="Some Event", start_date="2025-12-01", start_time="10:00")

    result = await delete_calendar_event(event_title="Nonexistent Event")
    assert "Event 'Nonexistent Event' not found" in result


@pytest.mark.asyncio
async def test_delete_calendar_event_from_nonexistent_calendar(temp_calendar_path):
    """Test deleting event from non-existent calendar."""
    result = await delete_calendar_event(event_title="Test Event", calendar_name="nonexistent")
    assert "Calendar 'nonexistent' does not exist" in result


@pytest.mark.asyncio
async def test_multiple_events_same_calendar(temp_calendar_path):
