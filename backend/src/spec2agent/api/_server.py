...
class DevServer:
    ...
    def __init__(
        ...
        self.port = port
        self.host = host
        # Default CORS origins include common frontend dev ports
    ...
    async def _ensure_executor(self) -> AgentFrameworkExecutor:
        ...
        if self.executor is None:
            ...
            logger.info(
                f"Discovered {len(discovered_entities)} entities from directory"
            )
            ...
            if self._pending_entities:
                ...
                for entity in self._pending_entities:
                    try:
                        entity_info = await discovery.create_entity_info_from_object(
                            entity, source="in-memory"
                        )
    ...
    async def _cleanup_entities(self) -> None:
        ...
        for entity_info in entities:
            try:
                entity_obj = self.executor.entity_discovery.get_entity_object(
                    entity_info.id
                )
    ...
    def create_app(self) -> FastAPI:
        ...
        # UI is now served separately by Vite dev server or static hosting
        # API-only server - no static file mounting
    ...
    def _register_routes(self, app: FastAPI) -> None:
        ...
        async def discover_entities() -> DiscoveryResponse:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Entity listing failed: {e!s}"
                ) from e
            ...
            try:
                ...
                if entity_info.type == "workflow":
                    ...
                    if entity_obj:
                        ...
                        workflow_dump = None
                        if hasattr(entity_obj, "to_dict") and callable(
                            getattr(entity_obj, "to_dict", None)
                        ):
                            try:
                                workflow_dump = entity_obj.to_dict()  # type: ignore[attr-defined]
                            except Exception:
                                workflow_dump = None
                        elif hasattr(entity_obj, "to_json") and callable(
                            getattr(entity_obj, "to_json", None)
                        ):
                            try:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Could not extract input info for workflow {entity_id}: {e}"
                            )
                        else:
                            if start_executor:
                                start_executor_id = getattr(
                                    start_executor, "executor_id", ""
                                ) or getattr(start_executor, "id", "")

                                message_types = _extract_executor_message_types(
                                    start_executor
                                )
                                input_type = _select_primary_input_type(message_types)

                                if input_type:
                                    input_type_name = getattr(
                        ...
                        if hasattr(entity_obj, "executors") and entity_obj.executors:
                            executor_list = [
                                getattr(ex, "executor_id", str(ex))
                                for ex in entity_obj.executors
                            ]
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to get entity info: {e!s}"
                ) from e
        ...
        async def add_entity(request: dict[str, Any]) -> dict[str, Any]:
            ...
            try:
                ...
                (
                    entity_info,
                    error_msg,
                ) = await executor.entity_discovery.fetch_remote_entity(url, metadata)

                if not entity_info:
                    # Sanitize error message - only return safe, user-friendly errors
                    logger.error(
                        f"Failed to fetch or validate entity from {url}: {error_msg}"
                    )
                    safe_error = (
                        error_msg if error_msg else "Failed to fetch or validate entity"
                    )
                ...
                raise HTTPException(
                    status_code=500,
                    detail="An unexpected error occurred while adding the entity",
                ) from e
        ...
        async def remove_entity(entity_id: str) -> dict[str, Any]:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error closing entity {entity_id} during removal: {e}"
                    )
                ...
                if success:
                    return {"success": True}
                raise HTTPException(
                    status_code=404, detail="Entity not found or cannot be removed"
                )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to remove entity: {e!s}"
                ) from e

        @app.post("/v1/responses")
        async def create_response(
            request: AgentFrameworkRequest, raw_request: Request
        ) -> Any:
            ...
            try:
                ...
                logger.info(
                    f"Parsed request: model={request.model}, extra_body={request.extra_body}"
                )
                ...
                if not entity_id:
                    error = OpenAIError.create(
                        f"Missing entity_id. Request extra_body: {request.extra_body}"
                    )
                ...
                try:
                    entity_info = executor.get_entity_info(entity_id)
                    logger.info(
                        f"Found entity: {entity_info.name} ({entity_info.type})"
                    )
                except Exception:
        ...
        async def create_thread(request_data: dict[str, Any]) -> dict[str, Any]:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to create thread: {e!s}"
                ) from e
        ...
        async def list_threads(agent_id: str) -> dict[str, Any]:
            ...
            try:
                ...
                for thread_id in thread_ids:
                    threads.append(
                        {"id": thread_id, "object": "thread", "agent_id": agent_id}
                    )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to list threads: {e!s}"
                ) from e
        ...
        async def get_thread(thread_id: str) -> dict[str, Any]:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to get thread: {e!s}"
                ) from e
        ...
        async def delete_thread(thread_id: str) -> dict[str, Any]:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to delete thread: {e!s}"
                ) from e
        ...
        async def get_thread_messages(thread_id: str) -> dict[str, Any]:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to get thread messages: {e!s}"
                ) from e
    ...
    async def _stream_execution(
        ...
        try:
            ...
            async for event in executor.execute_streaming(request):
                ...
                elif hasattr(event, "to_json") and callable(
                    getattr(event, "to_json", None)
                ):
                ...
                elif hasattr(event, "to_dict") and callable(
                    getattr(event, "to_dict", None)
                ):
    ...
    # UI mounting removed - frontend now served separately via Vite dev server or static hosting
